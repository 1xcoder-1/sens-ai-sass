...
The Roadmap table stores AI-generated career development roadmaps.
...
| description | String | Required | Roadmap description |
| steps | Json[] | Required | Array of roadmap steps with details |
| progress | Json | Required | Progress tracking for each step |
| createdAt | DateTime | Required | Record creation timestamp |
| updatedAt | DateTime | Required | Record last update timestamp |
...
4. **User → Roadmap**: One-to-many relationship. Each user can create multiple roadmaps.
5. **User → IndustryInsight**: Many-to-one relationship through the `industry` field in User table.
